...
from src.model.model_wrapper import ExpertPlacementManager, MoEGPT
...
def main():
    ...
    if args.use_topomoe:
        placement_manager_config = ExpertPlacementConfig(
            rebalance_steps= [25],         # trigger rebalance for these steps
            max_swap_iterations= 60,        # greedy iterations per rebalance
            min_improvement_frac= 0.005,    # stop if gain < 0.5 % per swap
            affinity_decay = 1,           # EMA decay for affinity matrix
            migrate_timeout_sec = 60.0      # safeguard for dist ops
        )
    model = MoEGPT(
    ...
    # grab the ep_group from the first MoE layer
    ...
    for batch in dataloader:
        ...
        if args.use_topomoe and step in placement_manager_config.rebalance_steps:
